...
**Resultado tras las correcciones — 8 de 12 repos 100% verdes** (build + matrix Java 21/25 + owasp + sbom + sonar, donde aplica), **mergeados a `main`:** `nova-java-date-utils`, `nova-bom`, `nova-java-api-standard`, `nova-java-mapper-utils`, `nova-java-observability-utils`, `nova-java-spring-boot-gradle-plugin`, `nova-java-spring-boot-archetype`, `nova-java-mask-utils` (este ultimo, bug A abajo, se cerro en una segunda pasada el mismo dia).
...
| B | `nova-java-observability-spring-boot-starter`, `nova-java-spring-boot-starter`, `nova-java-commons-spring-boot-starter` | `401 Unauthorized` al resolver dependencias de OTRO repo (`nova-observability-utils`, `nova-spring-boot-bom`, `nova-api-standard`) via GitHub Packages | El secret `NOVA_PACKAGES_READ_TOKEN` (PAT necesario para lectura cross-repo de paquetes — `GITHUB_TOKEN` solo puede leer paquetes del mismo repo) **sigue sin configurarse** (`gh secret list` solo muestra `NOVA_RELEASE_PAT`). | ⏸️ **Bloqueado, requiere accion del usuario.** Generar un PAT (`read:packages`) — ver seccion "Pendiente del usuario" mas abajo. Los 3 PRs quedan abiertos. |
...
1. **PAT `NOVA_PACKAGES_READ_TOKEN`** (unico bloqueante para 3 de los 4 PRs restantes): generar un GitHub Personal Access Token (classic, scope `read:packages` basta) y configurarlo como secret. Se necesita en 4 repos como minimo (`nova-java-observability-spring-boot-starter`, `nova-java-spring-boot-starter`, `nova-java-commons-spring-boot-starter`, `nova-java-spring-boot-parent`), aunque podria configurarse en los 12 de una vez para prevenir el mismo problema a futuro. Decision del usuario (2026-07-12): el usuario lo configura directamente via `gh secret set NOVA_PACKAGES_READ_TOKEN --repo <owner>/<repo>` (no se comparte el valor del token). Una vez configurado, el bug B deberia resolverse directamente en los 3 repos Gradle.
2. **Republicar `nova-java-spring-boot-starter`** (bug D, ver diagnostico arriba): requiere cortar una version nueva (ej. `1.0.1`) — sin cambios de codigo, el source ya es correcto. Es una decision de release, no un simple commit; pendiente de que el usuario confirme si procede.

**Estado:** ✅ NOVA-SEMVER-19, 20, 21 **implementados y validados** (los 3 jobs nuevos funcionan correctamente en los 12 repos). **8/12 PRs mergeados.** Los 4 PRs restantes estan bloqueados por el PAT pendiente (3 de ellos) + la decision de republish del bug D (`spring-boot-parent`, adicionalmente al PAT).
...
- **D:** 🟡 **Diagnosticado (causa raiz confirmada retrospectivamente via §11.9.25, sin necesitar el PAT)** — `nova-java-spring-boot-starter:1.0.0` fue publicado durante la breve ventana en que `nova-spring-boot-bom` estuvo en `1.0.1` (workaround de un 409 Conflict, luego revertido a `1.0.0`); su POM publicado quedo con esa referencia fantasma y nunca se re-publico. Fix: cortar una version nueva del starter (sin cambios de codigo). Solo afecta a `spring-boot-parent`.
