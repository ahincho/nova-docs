...
| **Estado** | ✅ **Resuelto y verificado** en `observability-spring-boot-starter`, `commons-spring-boot-starter` (ambos submodulos) y `spring-boot-starter` (tras republicar el BOM a `1.0.1`, §11.9.25). Todos los consumidores resuelven correctamente. |
...
| **Versiones sincronizadas** | Las properties de `nova-bom/pom.xml` (`mask-utils.version`, `date-utils.version`, `mapper-utils.version`, `api-standard.version`) y de `nova-spring-boot-bom/pom.xml` (`mask-utils-spring-boot-starter.version`, `api-standard-spring-boot-starter.version`, `observability-spring-boot-starter.version`, `nova-spring-boot-starter.version`) se actualizaron a `1.0.0`, reflejando las versiones reales ya publicadas en GitHub Packages. **Nota:** tras la republicacion a `1.0.1` del BOM (§11.9.25), todas estas properties apuntan a `1.0.0` (las versiones correctas de los artefactos reales). |
...
| `nova-java-spring-boot-starter` | ✅ | ✅ (tras refrescar BOM a 1.0.1, §11.9.25) | **Completo** — `nova-spring-boot-starter:1.0.0` |

**Total consolidado de la sesion (incluyendo la demo inicial de §11.9.14):** **9 de 9 repos Gradle + `nova-bom` con release real publicado y verificado en GitHub Packages.** El ultimo bloqueo (`nova-java-mapper-utils`, §11.9.22) fue resuelto por el usuario refrescando el valor del secret `NOVA_RELEASE_PAT`, y la cadena se completo actualizando `nova-bom` a `1.0.1` para reflejar la version real disponible de `mapper-utils` (§11.9.25).
...
| 22 | `nova-bom` nunca publicado a ningun registro | Gap | ✅ Resuelto — publicado `1.0.0` (§11.9.21), **republicado como `1.0.1`** con `mapper-utils.version=1.0.0` (§11.9.25) |
...
| 24 | Replicacion del release real en 7 repos Gradle restantes | Validacion | ✅ **7/7 completados** (§11.9.23, §11.9.25); todos los repos Gradle + el BOM tienen un release real `1.0.0` (o `1.0.1` para el BOM) publicado en GitHub Packages |

#### 11.9.25. Republish del BOM a `1.0.1` (workaround del 409 Conflict, 2026-07-10)

Una vez que `nova-mapper-utils:1.0.0` estuvo disponible en GitHub Packages (despues del fix del secret, §11.9.22), se intento republicar `nova-bom` con la property `mapper-utils.version` corregida de `0.1.0-SNAPSHOT` a `1.0.0`. Esto fallo con `409 Conflict` de GitHub Packages, ya que Maven Central y GitHub Packages son registros inmutables: **no es posible re-deployar la misma coordenada** (`groupId:artifactId:version`), hay que bumpear la version. Solucion aplicada:

1. `nova-bom/pom.xml`: `<version>1.0.0</version>` → `<version>1.0.1</version>` (y los 3 `<parent>` refs de los sub-BOMs).
2. Republish via `gh workflow run publish.yml` → exito. `nova-bom:1.0.1` y `nova-spring-boot-bom:1.0.1` ahora son los artefactos correctos a consumir.
3. `nova-java-spring-boot-starter/build.gradle.kts`: `platform("pe.edu.nova.java:nova-spring-boot-bom:1.0.0")` → `...:1.0.1`. Tag `v1.0.0` movido al nuevo commit. `Publish on Tag` exitoso → `nova-spring-boot-starter:1.0.0` publicado.

**Nota:** Las versiones `1.0.0` de `nova-bom` y `nova-spring-boot-bom` **siguen existiendo** en GitHub Packages (con contenido obsoleto: `mapper-utils=0.1.0-SNAPSHOT` que nadie puede resolver). No fueron eliminadas porque GitHub no expone una API de delete simple para packages Maven desde el agente. **Politica recomendada:** los consumidores nuevos deben usar `1.0.1+` (la unica que apunta a artefactos reales publicables). Si en el futuro alguien necesita limpiar, puede hacerlo desde la UI de GitHub Packages con permisos de admin.
...
15. **NOVA-SEMVER-15:** ✅ Primer release de prueba: `1.0.0` (no `0.1.0`, ver §11.8.3) ejecutado con exito de punta a punta y **replicado en 9 de 9 repos Gradle + el BOM** (push commits Conventional → release-please crea PR → merge → tag v1.0.0 → publish, 100% automatico con PAT real). Verificado con GitHub Release + paquetes reales publicados en GitHub Packages en todos los casos exitosos. La cadena final (`mapper-utils` + `spring-boot-starter`) se completo tras refrescar el secret `NOVA_RELEASE_PAT` (§11.9.22) y republicar el BOM a `1.0.1` (§11.9.25).
16. **NOVA-SEMVER-16:** ✅ Publicar `1.0.0` a GitHub Packages: **hecho** para 9 de 9 repos Gradle + el BOM. Verificar consumo desde el BOM: **confirmado end-to-end** — `nova-java-spring-boot-starter` resuelve correctamente `nova-spring-boot-bom:1.0.1`, `nova-date-utils:1.0.0`, `nova-mapper-utils:1.0.0`, `nova-mask-starter:1.0.0` y `nova-api-standard-starter:1.0.0` a traves del BOM (§11.9.17, §11.9.23, §11.9.25).
...
| Release automation | Bump manual via PR labels | Idem | ✅ Flujo 100% automatico validado end-to-end y replicado en 9/9 repos Gradle + el BOM (NOVA-SEMVER-15, 2026-07-10, todos en `1.0.0`, incl. tag→publish sin intervencion manual con PAT real, §11.9.14, §11.9.23, §11.9.25). 25 hallazgos (bugs, gaps y limitaciones) documentados en el proceso, todos resueltos o descartados (§11.9) | `release-please` PR + aprobacion humana | Idem |
| Publicacion | GitHub Packages unico (con placeholder `OWNER`) | GitHub Packages (con `ahincho` corregido) | ✅ GitHub Packages (con `ahincho`, 9 repos + 2 submodules commons-starter con `maven-publish`, + `nova-bom`). **11 paquetes reales publicados en `1.0.0`** (todos los repos Gradle con `maven-publish`): `nova-date-utils`, `nova-api-standard`, `nova-mask-utils`, `nova-mapper-utils`, `nova-observability-utils`, `nova-observability-starter`, `nova-spring-boot-gradle-plugin`, `nova-mask-starter`, `nova-api-standard-starter`, `nova-spring-boot-starter`, mas `nova-bom:1.0.1`/`nova-spring-boot-bom:1.0.1` (republicados tras §11.9.25; las versiones `1.0.0` de los BOMs existen pero tienen contenido obsoleto) | + Maven Central + Nexus (multi-registry workflows listos) | Idem |
...
| **NOVA-SEMVER-15** (primer release end-to-end) | ✅ **COMPLETADO** (replicado en 9/9 repos Gradle + el BOM, ver §11.9.10, §11.9.14, §11.9.22-25) | Nada. La evidencia original (`api-standard`, run `29024268916`) quedo inaccesible tras la recreacion de repos de NOVA-SEMVER-31 (§11.9.9), pero se genero evidencia nueva y mas completa en los 9 repos Gradle + el BOM, incluyendo la validacion del flujo 100% automatico con PAT real. 25 hallazgos adicionales descubiertos y documentados (§11.9.1-11.9.25). |
...
- ✅ **NOVA-SEMVER-15 (validado end-to-end con PAT real, replicado en 9/9 repos Gradle + el BOM):** Primer release end-to-end ejecutado con exito en `nova-devops`, `nova-java-date-utils`, `nova-java-api-standard`, `nova-java-mask-utils`, `nova-java-observability-utils`, `nova-java-spring-boot-gradle-plugin`, `nova-java-observability-spring-boot-starter`, `nova-java-commons-spring-boot-starter` (2 submodulos), `nova-java-mapper-utils` y `nova-java-spring-boot-starter`, mas `nova-bom` (todos `1.0.0` para los artefactos, `1.0.1` para el BOM tras el republish descrito en §11.9.25). La evidencia original (`api-standard`, run `29024268916`, 2026-07-09) quedo inaccesible tras la recreacion de repos de NOVA-SEMVER-31, y ese repo especifico quedo ademas con `release-please` roto (§11.9.12, **resuelto**). Los 4 bugs de §11.7 seguían aplicando; se encontraron **19 hallazgos adicionales** en esta sesion (documentados en §11.9):
  ...
  - Tras desbloquear `mapper-utils`, el BOM (publicado como `1.0.0` con la property `mapper-utils.version=0.1.0-SNAPSHOT`) tuvo que ser **republicado como `1.0.1`** porque GitHub Packages rechaza re-deployar la misma coordenada con HTTP 409 Conflict (registry inmutable). Esto requirio bumpear la version del BOM y actualizar el consumidor `spring-boot-starter` a `1.0.1` (§11.9.25).
...
**Publicaciones totales:** 9 (repos simples) + 2 (submodules commons-starter) = **11 publicaciones distintas** a GitHub Packages, mas `nova-bom`/`nova-spring-boot-bom` (2 adicionales). **Las 13 publicaciones reales verificadas en `1.0.0`** (todos los repos Gradle con `maven-publish` + los 2 BOMs): `nova-date-utils`, `nova-api-standard`, `nova-mask-utils`, `nova-mapper-utils`, `nova-observability-utils`, `nova-observability-starter`, `nova-spring-boot-gradle-plugin`, `nova-mask-starter`, `nova-api-standard-starter`, `nova-spring-boot-starter`, mas `nova-bom:1.0.1`/`nova-spring-boot-bom:1.0.1` (republicados tras §11.9.25; las versiones `1.0.0` de los BOMs existen pero tienen contenido obsoleto con `mapper-utils.version=0.1.0-SNAPSHOT`).
...
- ~~**Test del primer release**~~ — **Flujo 100% automatico validado end-to-end y replicado en 9 de 9 repos Gradle + el BOM** (ver §11.9.14, §11.9.22-25): merge del PR de release-please → tag creado con el PAT real → `publish-on-tag.yml` disparado automaticamente sin intervencion manual → artefacto publicado en GitHub Packages. En el proceso se encontraron y corrigieron 19 hallazgos adicionales (3 modulos con `publishing{}` incompleto, gap de resolucion de dependencias cross-repo, 3 bugs en `nova-devops` nunca antes ejercitados, `nova-bom` nunca publicado, secret corrupto en `mapper-utils`, republish del BOM a `1.0.1` por el 409 Conflict — ver §11.9.16-25). **Cadena completa, todos los repos publicando.**
...
- (Nuevo, 2026-07-10) **BOMs `nova-bom:1.0.0` y `nova-spring-boot-bom:1.0.0` quedaron con contenido obsoleto** (apuntan a `nova-mapper-utils:0.1.0-SNAPSHOT`). Fueron republicados como `1.0.1` (§11.9.25) tras el fix del secret de `mapper-utils`, pero las versiones `1.0.0` siguen existiendo en el registry. **Accion opcional:** eliminarlas desde la UI de GitHub Packages cuando sea posible (GitHub no expone una API de delete simple desde el agente).
...
2. (Opcional, mejora de calidad) Considerar eliminar las versiones obsoletas `nova-bom:1.0.0` y `nova-spring-boot-bom:1.0.0` desde la UI de GitHub Packages — ver nota arriba.
