...
**Pendiente del usuario (unico repo que sigue bloqueado: `nova-java-spring-boot-parent`):**
1. **Secret de lectura para `nova-java-spring-boot-parent`:** este repo es el unico de los 12 sin `NOVA_RELEASE_PAT` (nunca tuvo release-please configurado), asi que el fallback automatico no aplica aqui. El usuario puede: (a) agregar `NOVA_RELEASE_PAT` a este repo tambien (reusando el mismo token que ya tiene guardado para los demas), o (b) generar un PAT dedicado (`read:packages` basta) como secret `NOVA_PACKAGES_READ_TOKEN`. Decision del usuario (2026-07-12): lo configura el mismo via `gh secret set --repo ahincho/nova-java-spring-boot-parent` (no se comparte el valor del token).
2. **Republicar `nova-java-spring-boot-starter`** (bug D, ver diagnostico arriba): requiere cortar una version nueva (ej. `1.0.1`) — sin cambios de codigo, el source ya es correcto. Es una decision de release, no un simple commit; pendiente de que el usuario confirme si procede. Sin este fix, `spring-boot-parent` seguira bloqueado incluso despues del punto 1.

**Estado:** ✅ NOVA-SEMVER-19, 20, 21 **implementados y validados** (los 3 jobs nuevos funcionan correctamente en los 12 repos). **11/12 PRs mergeados.** Unico PR restante: `nova-java-spring-boot-parent#1`, bloqueado por los 2 puntos de arriba (secret + decision de republish).
...
19. **NOVA-SEMVER-19:** ✅ Activado `reusable-build-matrix.yml` (Java 21 + 25) en los 12 repos con CI. Validado con PRs reales: 11/12 100% verdes y mergeados (§11.9.30/31).
...
**Prioridad siguiente:** ninguna actividad de roadmap abierta salvo el backlog opcional (NOVA-SEMVER-30). El foco pasa a las 2 acciones pendientes del usuario (PAT + decision de republish, detalladas abajo) para cerrar los ultimos 4 PRs de Sprint 4, y eventualmente a un roadmap separado para los repos NestJS.
...
- **D:** 🟡 **Diagnosticado (causa raiz confirmada retrospectivamente via §11.9.25, sin necesitar el PAT)** — `nova-java-spring-boot-starter:1.0.0` fue publicado durante la breve ventana en que `nova-spring-boot-bom` estuvo en `1.0.1` (workaround de un 409 Conflict, luego revertido a `1.0.0`); su POM publicado quedo con esa referencia fantasma y nunca se re-publico. Fix: cortar una version nueva del starter (sin cambios de codigo). Solo afecta a `spring-boot-parent`, unico repo aun bloqueado.
...
- Queda **1 PR abierto sin mergear** (`nova-java-spring-boot-parent#1`), bloqueado por (i) falta de un secret de lectura propio (unico repo sin `NOVA_RELEASE_PAT`) y (ii) la decision de republish del bug D.
