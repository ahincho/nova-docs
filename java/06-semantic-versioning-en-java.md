...
| D | `nova-java-spring-boot-parent` (via `nova-java-spring-boot-starter:1.0.0` publicado) | Tras fijar C, `mvn verify` progresa mucho mas (matrix/sbom/owasp ya pasan) pero `build` sigue fallando: `Failed to read artifact descriptor for pe.edu.nova.java.starters:nova-spring-boot-starter:jar:1.0.0` → `Could not find artifact pe.edu.nova.java:nova-spring-boot-bom:pom:1.0.1` | El POM **publicado** de `nova-spring-boot-starter:1.0.0` en GitHub Packages parece referenciar `nova-spring-boot-bom` version **`1.0.1`** — version que no existe (`gh api .../versions` solo lista `1.0.0`) y que tampoco aparece en el codigo fuente actual (`build.gradle.kts` usa `api(platform("...:nova-spring-boot-bom:1.0.0"))` literal). Posible variante del mismo bug de "Gradle publica metadata con version resuelta/incorrecta" ya visto y corregido para los BOMs en su momento (§11.9.25/26), esta vez en el POM publicado del starter. **No se pudo confirmar la causa exacta**: ni mi propio `gh auth token` tiene scope `read:packages`, asi que no pude descargar el POM publicado real para inspeccionarlo directamente. | 🔴 **NUEVO, no diagnosticado con certeza.** Requiere el mismo PAT que el bug B para poder inspeccionar el artefacto publicado y decidir el fix real (posiblemente editar y re-publicar `nova-java-spring-boot-starter`). |
...
**Pendiente del usuario (unico bloqueante real que queda de esta actividad):** generar un GitHub Personal Access Token (classic, scope `read:packages` basta) y configurarlo como secret `NOVA_PACKAGES_READ_TOKEN`. Se necesita en 4 repos como minimo (`nova-java-observability-spring-boot-starter`, `nova-java-spring-boot-starter`, `nova-java-commons-spring-boot-starter`, `nova-java-spring-boot-parent`), aunque podria configurarse en los 12 de una vez para prevenir el mismo problema a futuro. El usuario puede: (a) pegar el valor del token para que se configure via `gh secret set` en los repos que corresponda, o (b) ejecutar `gh secret set NOVA_PACKAGES_READ_TOKEN --repo <owner>/<repo>` el mismo. Una vez configurado, el bug B deberia resolverse directamente en los 3 repos Gradle; el bug D en `spring-boot-parent` requiere ademas inspeccionar el POM publicado para confirmar el fix exacto.

**Estado:** ✅ NOVA-SEMVER-19, 20, 21 **implementados y validados** (los 3 jobs nuevos funcionan correctamente en los 12 repos). **8/12 PRs mergeados.** Los 4 PRs restantes (bug B x3 + bug D) estan bloqueados unicamente por el PAT `NOVA_PACKAGES_READ_TOKEN` pendiente del usuario.
